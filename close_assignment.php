...
try {
    ...
    $update_query = "UPDATE student_assignments_v1 SET assigned = NULL WHERE student_id = " . $student_id;
    $updated_result=$conn->query($update_query);

    // if task completed successfully, add to 'completed' column
    if (isset($_POST["completed"])) {
        $complete_query = "UPDATE student_assignments_v1 SET completed = '" . $worker_id . "' WHERE student_id = " . $student_id;
